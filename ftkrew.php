<?php
$status = "open";
$GLOBALS['page_title'] = 'FTKrew | Students | Dance Marathon at UF';
$GLOBALS['parent'] = 'get-involved';
include("includes/head.php");
include("includes/navbar.php");
?>

<style>
.test{
	color: #002e60;
}
</style>

<div class="page-heading parallax FTKrew">
	<div class="inner-wrapper">
		<div class="container">
			<div class="row">
				<div class="col-md-12">
					<h1>FTKrew</h1>
				</div>
			</div>
		</div>
	</div>
</div>

<div class="page-content">
	<div class="container">
		<div class="row">
			<div class="col-md-3">
				<div class="sub-nav">
          <ul>
						<li><a href="/alumni">Alumni</a></li>
						<li><a href="/applications">Applications</a></li>
						<li><a href="/captain-teams">Captain Teams</a></li>
						<li><a href="/dancers">Dancers</a></li>
						<li><a href="/delegates">Delegates</a></li>
						<li><a href="/emerging-leaders">Emerging Leaders</a></li>
						<li><a class="active">FTKrew</a></li>
						<li><a href="/organizations">Organizations</a></li>
          </ul>
				</div>
			</div>
			<div class="col-md-8 col-md-push-1">
				<div role="tabpanel">

					<!-- Nav tabs -->
					<ul class="nav nav-tabs" role="tablist">
						<li role="presentation" class="active"><a href="#FTKrew" aria-controls="FTKrew" role="tab" data-toggle="tab">FTKrew</a></li>
          <!--  <li role="presentation"><a href="#Marathon-FTKrew" aria-controls="Marathon-FTKrew" role="tab" data-toggle="tab">Marathon FTKrew</a></li>
          <li role="presentation"><a href="#Morale-FTKrew" aria-controls="Morale-FTKrew" role="tab" data-toggle="tab">Morale FTKrew</a></li> -->
      </ul>

      <!-- Tab panes -->
      <div class="tab-content">
      	<div role="tabpanel" class="tab-pane fade in active" id="FTKrew">
      		<h3>FTKrew</h3>
					<p>FTKrew consists of <span class="test">motivated<span> individuals who volunteer at Dance Marathon's main event in the spring semester. FTKrew offers a great opportunity for people to get involved with Dance Marathon, participate in service, and attend the event! Selected FTKrew members will attend two meetings in preparation for the event in March. FTKrew members will be assigned shifts during the event with specific duties and responsibilities to assist the Dancer Relations, Productions, and Hospitality teams. FTKrew members are an essential part of ensuring the efficiency and success of the 26.2-hour event.</p>


					<p><i>Selected FTKrew members will attend two meetings in preparation for the event in March. Each member will be assigned shifts during the event with specific duties and responsibilities assisting the Dancer Relations, Productions, and Hospitality teams.</i></p>

					<p>FTKrew Applications are open from Monday, February 19th through Sunday, February 25th! Apply through the following <a href="https://ufl.qualtrics.com/jfe/form/SV_bNOFhuVnlNQWdi5">link.</a></p>
      	</div>

      <div role="tabpanel" class="tab-pane fade" id="Marathon-FTKrew">
            	<h3></h3>
							<table style="width:100%">

			      	</table>
			      </div>
            <div role="tabpanel" class="tab-pane fade" id="Morale-FTKrew">
			      	<h3>Morale FTKrew</h3>
							<table style="width:100%">

							</table>
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>
</div>

<?php include("includes/foot.php"); ?>
